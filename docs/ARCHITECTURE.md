...
- **Structured import handoff** (import.js `importFromPayload` + app.js `#import=` hash route): the no-clipboard path for CountTooling. URL: `<app>/#import=<base64 JSON>`. **v2** payload: `{v:2, source, project?:{name?, plansUrl?}, items:[{description, quantity|count, unit?:'ea'|'ft'|'px', type?, pages?|page?, group?, meta?, children?:[{description, quantity, unit?, type?}]}]}` — CountTooling states facts, nothing it provided is inferred; an invalid `type` falls back to inference, a missing `unit` reads the name convention, then `ea`. **v1** (`{v:1, source, items:[{description, count, page?, type?}]}`) still imports. Both land in the same preview modal; the hash is stripped first. Checked-in contract: `import-files/counttooling-export.fixture.txt` (asserted by import.test.js and import-fixture.spec.js; the CountTooling repo asserts its exporter still produces it).

...
- **Quality gate**: `npm run check` (eslint `--max-warnings 0` + `node --test` unit tests) and `npx playwright test` (browser smoke specs; starts the dev server itself). Run both before committing. Naming: `*.test.js` = node:test units, `*.spec.js` = Playwright. ESLint ignores `.claude/**` (Claude Code worktrees are full repo copies) and `vendor/**`.
- **User-facing errors are toasts, never `alert()`**: `TakeoffUtils.toast(message, { kind: 'info'|'success'|'error' })` renders into `#toast-region`. `confirm()` stays for destructive confirms.
...
| `js/handoff.js` | "Copy for PipeTooling": the manifest as PipeTooling's Counts-import text (v0 of the onward seam; unit-tested in `handoff.test.js`) |
...
- **Project settings**: `laborRate` ($/hr) and **`taxRate`** (a PERCENT, default 8.25 — Texas' combined maximum; `null` = default) live on the project document with **`plansUrl`** (the CountTooling view link the counts came from, shown in the header and forwarded to PipeTooling).
- **Persistence**: all durable writes go through `TakeoffStorage` (js/storage.js). Two debounced (400 ms) documents: the open PROJECT (`{v:1, id, savedAt, name, manifest, laborRate, taxRate?, plansUrl?}`) and the account-level BOOK (`{v:1, savedAt, laborBook, laborBookMeta}`); assemblies write immediately. Projects mirror to the `takeoff_projects` Supabase table (schema-aligned with Count Tooling's `projects`; SQL in `supabase/`). The legacy `takeoff-workspace` key migrates into project #1 on first boot and stays behind as a frozen backup. Share links (`v:2` envelope, now with `name`) import into a NEW project.

...
- **Project settings**: `laborRate` ($/hr) and **`taxRate`** (a PERCENT, default 8.25 — Texas' combined maximum; `null` = default) live on the project document with **`plansUrl`** (the CountTooling view link the counts came from, shown in the header and forwarded to PipeTooling), **`externalRef`** (the PipeTooling bid number, a header chip), **`reviewStatus`** (`draft` | `ready` | `changes` | `reviewed`, with `reviewNote`; a header chip + a select in Manage Projects) and **`agentImport`** (the agent door's provenance, null for hand-built projects). The cloud row keeps `external_ref` / `review_status` / `review_note` / `agent_import` as COLUMNS (004) so PipeTooling's bridge reads them; the client falls back to base columns until 004 is applied.

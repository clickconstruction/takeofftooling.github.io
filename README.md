...
- **Sales tax (%)**: The job's own rate, typed beside the labor rate and saved with the bid (a new bid starts at 8.25%, Texas' combined maximum; bids saved before the box existed keep the 8.5% they were bid at). It travels with a share link, so both people read the same grand total.
...
- each row gets a **type** from its name and unit (`1/2" EMT` → Conduit, `MC 12/2` → Wire, `Type A` → Lighting, `Panel LP-1` → Gear, `Data Drop` → Special Systems); a line the app can't type shows a dashed **Needs a type** picker — set it in the preview instead of after the import
- a count the app can't read shows as **× ?** and is never guessed as 0 or 1

A **preview modal** shows what is on the bid beside what the count says. **The count's numbers are totals, not additions.** A line whose description and unit already exist on the bid sets that row to the import's number — up or down — and takes its plan page; the row's price and labor are left alone, and its children are matched the same way underneath it. Lines that don't exist yet are added.
...
- **Print for review** — the bid as a reviewer reads it: page 1 is the same summary the screen shows (materials by type, sales tax at the job's own rate, hours by type, labor rate, other charges, grand total — dollars and cents), then the line items with quantity (with its unit), hours each and total, price each and total, and the plan page. Its labor total is the screen's labor total; an unscaled (px) row prints its pixel length and no hours or money.
...
- **Copy share link** (Print Options) — copies a link carrying the full manifest, **the labor rate, the sales tax rate, the job details and the plans link**, so whoever opens it sees the same grand total you do. It lands in a new bid on their device, above a line saying whose bid it is and when it was shared; their own bids keep their own rate. Opening the same link twice reopens the one copy instead of making a second.
- PDFs are named after the job and the day: `northgate-retail-purchase-list-2026-09-07.pdf`. All three wrap long descriptions, paginate with the header row repeated, and stamp the project name, date and "Page n of N" on every page.

### Totals

The summary below the table is a **cost summary**: materials by type, sales tax at the project's rate (editable in the summary; a new bid starts at 8.25%, and a bid saved before the rate was editable keeps the 8.5% it was bid at), labor hours × the project's labor rate, other charges (permits, power company, temporary power, and site work — trenching and rentals are untaxed and never on the purchase list). Margin and the bid price are set in PipeTooling.

### Labor and Price Book

Two sides, toggled at the top of the modal:

- **Parts** — Editable per-tab sections (labor hours + price per item, each price badged with who supplied it and how old it is), woven together with the live supplier catalog: supplier categories render as regular sections (merging into a curated section or group of the same name), with a per-tab filter that searches both together. The Devices tab ships with receptacles, switches, occupancy sensors, boxes & rings (the 1900 box, 4" square, 4-11/16", octagon, handy and gang boxes, mud rings and covers), covers & plates and MC/romex connectors; Lighting carries troffers, downlights, exit & emergency, fixture whips and photocells; Special Systems carries fire alarm, data and security & AV starters; Gear carries disconnects 30A–600A; Wire carries NM-B (romex) and MC cable by the foot; Conduit carries pull string, mule tape and pull rope. The curated starters carry MC-book hours and prices (badged *MC book*); the piece rows ship with hours and no price on purpose — the supply house is where today's price comes from, and this book is where the hours live
